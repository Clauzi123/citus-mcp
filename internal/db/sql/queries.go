...
const (
    ...
    QueryPgDistNodeStatus  = "SELECT nodeid, nodename, nodeport, noderole, COALESCE(isactive, true) AS isactive, COALESCE(shouldhaveshards, false) AS shouldhaveshards FROM pg_dist_node ORDER BY nodeid"
