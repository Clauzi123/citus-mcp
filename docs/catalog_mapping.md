...
| `pg_dist_partition` | table | Table distribution metadata | `list_distributed_tables`, `table_inspector`, `citus_colocation_inspector` |  |
...
| `citus_shard_indexes_on_worker` | view | Index presence on workers | `table_inspector` (index drift detection) | ✅ Implemented |
| `citus_tables`, `citus_schemas` | view | Table/schema distribution info | `list_distributed_tables` | Potential `citus_schema_overview` |
| `pg_dist_colocation` | table | Colocation groups | `list_distributed_tables`, `citus_colocation_inspector` | ✅ Implemented |
| `pg_dist_background_job`, `_task`, `_depend` | table | Background jobs (rebalance, copy) | `citus_advisor` (ops focus), `citus_job_inspector` | ✅ Implemented |
...
| `citus_stat_activity`, `citus_dist_stat_activity` | view | Cluster-wide activity | `citus_advisor` (ops focus), `citus_activity` | ✅ Implemented |
...
| --- | --- | --- |
| `citus_activity` | ✅ Implemented | Cluster-wide activity monitor using `citus_stat_activity` |
| `citus_job_inspector` | ✅ Implemented | Background job progress (rebalance, copy operations) |
| `citus_colocation_inspector` | ✅ Implemented | Colocation group analysis with table details |

## Gaps & Priorities

1. ~~**Lock & wait visibility** — `citus_lock_inspector`~~ ✅ Already exists
2. ~~**Activity dashboard** — `citus_activity`~~ ✅ Implemented
3. ~~**Background jobs** — `citus_job_inspector`~~ ✅ Implemented
4. **Query hotspots** — `citus_top_statements` (use `citus_stat_statements`)
5. **Tenant hotspots** — `citus_tenant_hotspots`
6. **Pool health** — `citus_pool_inspector`
7. ~~**Index drift** — enhance `table_inspector`~~ ✅ Implemented
